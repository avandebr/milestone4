...
public class QuadGraph {
	 ...
	 */ // BUT THIS METHOD DIDN'T WORK CORRECTLY !!
	public static boolean validArea(PVector c1, PVector c2, PVector c3, PVector c4, float max_area, float min_area){
		
		double area = getArea(c1, c2, c3, c4);
		
		return (area < max_area && area > min_area);
	}

	public static double getArea(PVector c1, PVector c2, PVector c3, PVector c4){
		PVector l1 = PVector.sub(c2, c1), l2 = PVector.sub(c3, c2), l3 = PVector.sub(c4, c3), l4 = PVector.sub(c1, c4);
		double a = c1.dist(c2), b = c2.dist(c3), c = c3.dist(c4), d = c4.dist(c1);
		double A = Math.acos(PVector.dot(l1, l2)/(a*b)), C = Math.acos(PVector.dot(l3, l4)/(c*d));
		return 0.5*(a*b*Math.sin(A) + c*d*Math.sin(C));
	}
